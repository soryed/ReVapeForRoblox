...
local function hotbarSwitchV2(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		local args = {
		    [1] = {
		        ["hand"] = slot
	...
	airbender = function()
        ...
        pyro = function()
			...
			if plr and (not Legit.Enabled or (lplr.Character:GetAttribute("Health") or 0) > 0) then
				...
				bedwars.Client:Get("UseFlamethrower"):CallServer()
				Speed:Toggle(true)
				task.wait(1.85)
				Speed:Toggle(false)
            end
		...
		pinata = function()
			local REMOTE = nil
			if remotes.DepositCoins == nil or remotes.DepositCoins == "" then REMOTE = "DepositCoins" print('remotes.DepositCoins was nil') else REMOTE = remotes.DepositCoins print('remotes.DepositCoins wasnt nil') end
