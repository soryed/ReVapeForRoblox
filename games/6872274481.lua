...
if getgenv().Closest then
run(function()
	local Killaura
	local Targets, Sort, SwingRange, AttackRange, AngleSlider
	local UpdateRate, MaxTargets, SwingTime
	local Mouse, GUI, Limit, SyncHit
	...
	local function canAttack()
		...
		local sword = store.tools.sword
		if Limit.Enabled then
			if not sword or store.hand.toolType ~= "sword" then
				return false
			end
		end
	...
	local function getSwingDelay(meta)
		if SyncHit.Enabled then
			return math.max(0.08, (meta.sword.attackSpeed * 0.6 - 0.00125))
		end
		return math.max(0.11, meta.sword.attackSpeed)
	end

	local function humanDelay(base)
		return base + math.random(15, 45) / 1000 + math.random() * 0.02 + math.random()
	end

	local function cpsDelay()
		local MIN_CPS, MAX_CPS = 6, 9
		return 1 / math.random(MIN_CPS, MAX_CPS)
	end

	local function softRange(range)
		return range - math.random(10, 35) / 100
	end

	local function shouldMiss()
		return math.random(1, 12) == 1 
	end

	local function validTarget(root, targetRoot)
		...
		local flatDir = delta.Unit * Vector3.new(1,0,1)
		local dot = math.clamp(flatLook:Dot(flatDir), -1, 1)
		local angle = math.deg(math.acos(dot))

		return angle <= (AngleSlider.Value * 0.5)
	end
	...
	local function attackTarget(sword, meta, ent)
		local root = entitylib.character.RootPart
		local targetRoot = ent.Character and ent.Character.PrimaryPart
		if not targetRoot then return end

		local dir = (targetRoot.Position - root.Position).Unit
		local spoofPos = root.Position + dir * (AttackRange.Value - 0.15)

		store.attackReach = AttackRange.Value
		local num = (0.0085 + math.random())
		if num >= 1 then
			num = num - 1 + math.random()
		end
		store.attackReachUpdate = tick() + 1 - (num)
		bedwars.SwordController.lastAttack = workspace:GetServerTimeNow()
		...
		Function = function(enabled)
			...
			task.spawn(function()
				while Killaura.Enabled do
					local sword = canAttack()
					if sword and entitylib.isAlive then
						local meta = bedwars.ItemMeta[sword.tool.Name]
						local delay = math.max(humanDelay(getSwingDelay(meta)), cpsDelay())

						if tick() - lastAttack >= delay then
							local root = entitylib.character.RootPart

							for _, ent in getTargets() do
								if ent.RootPart and validTarget(root, ent.RootPart) and (not lastTarget or ent == lastTarget or tick() - targetSwitchTime > 0.4) then
									if shouldMiss() then
										lastAttack = tick()
										break
									end
									...
									local oldRange = AttackRange.Value
									AttackRange.Value = softRange(oldRange)
									attackTarget(sword, meta, ent)
									AttackRange.Value = oldRange
									break
								end
							end
						end
	...
	Mouse = Killaura:CreateToggle({Name = "Require mouse down"})
	GUI = Killaura:CreateToggle({Name = "GUI check"})
	Limit = Killaura:CreateToggle({Name = "Limit to sword"})
end)
else
run(function()
	local Killaura
	...
	local function getSwingDelay(meta)
		if SyncHit.Enabled then
			return math.max(0.08, (meta.sword.attackSpeed * 0.6 - 0.0085))
		end
		return math.max(0.11, meta.sword.attackSpeed)
	end
		...
		Function = function(enabled)
			...
			task.spawn(function()
				while Killaura.Enabled do
					...
					if sword and entitylib.isAlive then
						local meta = bedwars.ItemMeta[sword.tool.Name]
						local delay = getSwingDelay(meta)

						if tick() - lastAttack >= delay then
							for _, ent in getTargets() do
								if ent.RootPart and validTarget(entitylib.character.RootPart, ent.RootPart) then
									lastAttack = tick()
									Attacking = true
									store.KillauraTarget = ent
									attackTarget(sword, meta, ent)
	...
	elseif role == "user" then
		...
		SyncHit = Killaura:CreateToggle({
			Name = 'Sync Hit-Time',
			Tooltip = "more aggressive server synced swings",
			Default = false,
		})
	elseif role == "premium" then
		MaxRange = 24
