					...
					if entitylib.isAlive and ((not ClickAim.Enabled) or (tick() - bedwars.SwordController.lastSwing) < 0.4) then
						...
						if ent then
							...
							if Shake.Enabled then
								...
								gameCamera.CFrame = gameCamera.CFrame:Lerp(CFrame.lookAt(gameCamera.CFrame.Position, ent.RootPart.Position), speed)
							end
						end
					end
				end))
			...
			if callback then
				...
				bedwars.ProjectileController.calculateImportantLaunchValues = function(...)
					...
					if plr then
						...
						local calc = prediction.SolveTrajectory(newlook.p, projSpeed, gravity, plr[TargetPart.Value].Position, projmeta.projectile == 'telepearl' and Vector3.zero or plr[TargetPart.Value].Velocity, playerGravity, plr.HipHeight, plr.Jumping and 42.6 or nil, rayCheck,math.round(game.Players.LocalPlayer:GetNetworkPing() * 1000)))
						if calc then
						...
						if ent then
							...
							for _, data in getProjectiles() do
								...
								if (FireDelays[item.itemType] or 0) < tick() then
									...
									local calc = prediction.SolveTrajectory(pos, projSpeed, gravity, ent.RootPart.Position, ent.RootPart.Velocity, workspace.Gravity, ent.HipHeight, ent.Jumping and 42.6 or nil, rayCheck,math.round(game.Players.LocalPlayer:GetNetworkPing() * 1000)))
									if calc then
										targetinfo.Targets[ent] = tick() + 1
...
run(function()
	...
	local function firePearl(pos, spot, item)
		...
		local calc = prediction.SolveTrajectory(pos, meta.launchVelocity, meta.gravitationalAcceleration, spot, Vector3.zero, workspace.Gravity, 0, 0,math.round(game.Players.LocalPlayer:GetNetworkPing() * 1000)))
				...
				AutoToxic:Clean(vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill then
						...
						elseif killer == lplr and Toggles.Kill.Enabled then
							sendMessage('Kill', (killed.DisplayName or killed.Name), 'you should switch to onyx, my friend | <obj> !')
						end
					end
				end))
...
run(function()
	...
	SlientAura = vape.Categories.Combat:CreateModule({
		Name = "SlientAura",
		Tooltip = "Synchronizes swing timing with AimAssist making you look legit",
		Function = function(callback)
			if callback then
...
run(function()
	...
	local AutoKitFunctions = {
		ghost_catcher = function()
			kitCollection('ghost', function(v)
		...
		dino_tamer = function()
			local function getBedNear()
				local localPosition = entitylib.isAlive and entitylib.character.RootPart.Position or Vector3.zero
				for _, v in collectionService:GetTagged("bed") do
					if (localPosition - v.Position).Magnitude < Legit.Enabled and 25 or 45 then
			...
			repeat
				...
				if bed then
					if bedwars.AbilityController:canUseAbility('dino_charge') then
						bedwars.AbilityController:useAbility('dino_charge')
					end	
				end
				task.wait(.45)
		    until not AutoKit.Enabled
		...
		sheep_herder = function()
			kitCollection('sheep', function(v)
				replicatedStorage.rbxts_include.node_modules["@rbxts"].net.out._NetManaged["SheepHerder/TameSheep"]:FireServer(v)
			end, 18, false)
		end,
		regent = function()
			repeat
		...
		yeti = function()
			local function getBedNear()
				local localPosition = entitylib.isAlive and entitylib.character.RootPart.Position or Vector3.zero
				for _, v in collectionService:GetTagged("bed") do
					if (localPosition - v.Position).Magnitude < Legit.Enabled and 10 or 20 then
						if v:GetAttribute("Team" .. (lplr:GetAttribute("Team") or -1) .. "NoBreak") then 
							return nil 
						end
						return v
		...
		block_kicker = function()
				...
				bedwars.BlockKickerKitController.getKickBlockProjectileOriginPosition = function(...)
					...
					if plr then
						local calc = prediction.SolveTrajectory(origin, 100, 20, plr.RootPart.Position, plr.RootPart.Velocity, workspace.Gravity, plr.HipHeight, plr.Jumping and 42.6 or nil,nil, math.round(game.Players.LocalPlayer:GetNetworkPing() * 1000)))
...
if getgenv().TestMode or role == "owner" or role == "coowner" then
	run(function()
		local JumpHeights
		local Height 
		JumpHeights = vape.Categories.Blatant:CreateModule({
			Name = "Jump Heights",
			Tooltip = "Increases your jump height with an undetected method!",
			Function = function(callback)
				if callback then
		    		bedwars.JumpHeightController:getJumpModifier():addModifier({
                        airJumps = Height.GetRandomValue() or 1
                    })
				else
		    		bedwars.JumpHeightController:getJumpModifier():addModifier({
                        airJumps = 0
		...
		Height = JumpHeights:CreateTwoSlider({
			Name = "Air Jumps",
			Min = 1,
			Max = 5,
			DefaultMin = 1,
