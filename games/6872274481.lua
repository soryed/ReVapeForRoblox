...
run(function()
	...
	local remoteNames = {
		...
		SetInvItem = debug.getproto(require(replicatedStorage.TS.entity.entities['inventory-entity']).InventoryEntity.SetInvItem, 3),
...
run(function()
	...
	OldBedwars = vape.Categories.Render:CreateModule({
		Name = "GameTheme",
		Function = function(callback)
		if callback then
				if not transformed then
					...
					OldBedwars:Toggle(false)
				end
			else
				notif("GameTheme", "Disabled Next Game", 10,"warning")
			end
		end,
		ExtraText = function()
