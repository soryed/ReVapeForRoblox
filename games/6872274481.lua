...
run(function()
	TrapDisabler = vape.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
		Tooltip = 'Disables Snap Traps -- rewrite'
	})
end)
					...
					if sword then
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if delta.Magnitude > AttackRange.Value then continue end
								if delta.Magnitude < 14.4 and (SyncHit.Enabled and tick() - swingCooldown - 0.0465 or tick() - swingCooldown) < math.max(ChargeTime.Value, 0.02) then continue end
...
run(function()
    ...
    InvisibleCursor = vape.Categories.Utility:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                notif('Invisible Cursor', 'Cursor hiding enabled', 2,'warning')
            else
                isActive = false
    ...
    LimitToItems = InvisibleCursor:CreateToggle({
        Name = 'Limit to Bow',
        Tooltip = 'Only hide cursor when holding a bow/crossbow/headhunter',
        Function = updateCursor
    })
    ...
    InvisibleCursor:Clean(vapeEvents.InventoryChanged.Event:Connect(function()
        updateCursor()
    end))
...
run(function()
	local NoStreamer	
	NoStreamer = vape.Legit:CreateModule({
		Name = "No Streamer",
		Tooltip = "Allows you to be in streamer mode but unhides other players",
		Function = function(callback)
			if callback then
				task.spawn(function()
				...
				task.spawn(function()
					for _,v in pairs(playersService:GetPlayers()) do
						if v == lplr then continue end
						if tostring(v:GetAttribute("Disguised")) == "false" then
							v:SetAttribute("Disguised", true)
						end
					end
				end)
